...
@Component({
      ...
      <div class="container">
        ...
        <div class="courses-grid">
          @for (course of filteredCourses; track course.id) {
            <div class="course-card" [class.enrolled]="isEnrolled(course.id)" (click)="isEnrolled(course.id) ? openCourse(course) : goToPayment(course)" style="cursor: pointer;">
              ...
              <div class="card-content">
                ...
                <div class="card-footer">
                  ...
                  <div class="card-actions">
                    @if (isEnrolled(course.id)) {
                      <button class="btn btn-open-course" (click)="openCourse(course); $event.stopPropagation()">
                        <i class="ti ti-player-play"></i>
                        Go to Course
                      </button>
                    } @else {
                      @if (isInCart(course.id)) {
                        <button class="btn btn-added" (click)="$event.stopPropagation()" disabled>
                          <i class="ti ti-check"></i>
                          Added to Cart
                        </button>
                      } @else {
                        <button class="btn btn-cart" (click)="addToCart(course); $event.stopPropagation()">
                          <i class="ti ti-shopping-cart"></i>
                          Add to Panier
                        </button>
                      }
                      <button class="btn-enroll" (click)="goToPayment(course); $event.stopPropagation()">
  ...
  styles: [`
    ...
    .enrolled-badge { position: absolute; top: 16px; left: 16px; padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 600; background: #10b981; color: #fff; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.3); }
    .course-card.enrolled { border: 2px solid #10b981; }
    ...
    .btn { display: inline-flex; align-items: center; gap: 6px; padding: 10px 16px; border-radius: 10px; font-size: 13px; font-weight: 600; text-decoration: none; transition: all 0.3s ease; cursor: pointer; border: none; }
    .btn-cart { background: linear-gradient(135deg, #6366f1, #4f46e5); color: #fff; &:hover { transform: scale(1.05); box-shadow: 0 6px 16px rgba(99, 102, 241, 0.3); } }
    .btn-added { background: #10b981; color: #fff; cursor: not-allowed; opacity: 0.8; }
    .btn-open-course { background: linear-gradient(135deg, #10b981, #059669); color: #fff; &:hover { transform: scale(1.05); box-shadow: 0 6px 16px rgba(16, 185, 129, 0.3); } }
...
export class ClientCoursesListComponent implements OnInit {
  ...
  openCourse(course: Course): void {
    ...
    window.location.href = `/courses/${course.id}/content`;
  }

  goToPayment(course: Course): void {
    // Navigate to payment page with course details
    window.location.href = `/payment?courseId=${course.id}&courseTitle=${encodeURIComponent(course.title)}&coursePrice=${course.price}`;
  }
}
